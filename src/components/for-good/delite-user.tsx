'use client';
import { deleteUser } from '@/app/(private)/for-good/actions';
...
export function DeleteUser({ id, name }: { id: string; name: string }) {
    ...
    return (
        <>
            <button
                className="p-2 py-1 rounded bg-red-600 cursor-pointer"
                onClick={() => setOpenDelete(true)}
            >
                {isPending ? (
                    <Loader2 className="animate-spin" />
                ) : (
                    <Trash2 color="white" strokeWidth={2} size={16} />
                )}
            </button>
            ...
            {openDelete ? (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className="relative z-10 w-[calc(100%-32px)] max-w-md rounded-lg bg-white p-4 shadow-lg">
                        ...
                        <h2>
                            Удаление запеси о двери <strong>{name}</strong> ?
                        </h2>
